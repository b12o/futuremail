...
<template>
  <div class="nb-card">
    <h2 class="font-display text-2xl uppercase mb-5">New time capsule</h2>
    ...
    <form class="flex flex-col gap-4" @submit.prevent="submit">
      ...
      <div>
        ...
        <label class="inline-flex items-center gap-2 cursor-pointer select-none">
          ...
          <span class="font-bold text-sm uppercase tracking-wide">Seal contents (encrypted)</span>
        </label>
      </div>
      ...
      <div>
        ...
        <input
          ...
          :min="customMin"
        />
        <p v-if="error" class="nb-badge nb-badge-red mt-2 self-start">{{ error }}</p>
      </div>

      <p v-if="success" class="nb-sticker nb-badge-green !bg-nb-green self-start">
        {{ success }}
      </p>

      <button type="submit" class="nb-btn nb-btn-pink w-full md:w-auto md:self-end" :disabled="submitting">
